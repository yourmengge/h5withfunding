...
export class DepositComponent implements OnInit, OnDestroy {
  ...
  constructor(public data: DataService, public http: HttpService, public activeRoute: ActivatedRoute) {
    ...
    this.http.getManageFee().subscribe(res => {
      if (res['resultInfo']['CTRL_COLLECT_MANAGE_FEE_MODE'] !== '0' && this.type === 0) {
        this.needShow = false;
  ...
  ngOnInit() {
    if (this.data.logo === 'guge' || this.data.logo === 'daizong') {
      this.oneType = true;
    }
    this.data.clearInterval();
  ...
  judge() {
    if (this.money % 100 !== 0 || this.money === null || this.money <= 0) {
      // 信息框
      layer.open({
        content: '投入金额必须是100的倍数，且大于0'
        , skin: 'msg'
        , time: 2
      });
      return false;
  ...
  usercenter() {
    this.http.userCenter2().subscribe((res: DataService['userInfo']) => {
      this.userInfo = res;
      this.data.setSession('version', res['cashType'] === 10 ? 2 : 1);
      this.data.version = res['cashType'] === 10 ? 2 : 1;
      this.oneType = res['cashType'] === 10 ? true : false;
    ...
    this.http.userCenter2().subscribe((res: DataService['userInfo']) => {
      ...
      if (type) { // 增配
        if (this.judge()) {
          ...
          if (this.fee < 0 && this.data.version === 1) { // 普通用户
